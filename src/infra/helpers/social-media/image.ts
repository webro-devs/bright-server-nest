...
const CImage = async ({ txt, ctgs = [], imgPath, imgName }) => {
  ...
  try {
    ...
    const logo = await Jimp.read(path.resolve(process.cwd(), './uploads/logo.png'));
    const topMask = await Jimp.read(
      path.resolve(process.cwd(), './uploads/mask/top.png'),
    );
    const bottomMask = await Jimp.read(
      path.resolve(process.cwd(), './uploads/mask/bottom.png'),
    );
    const url = `${imgPath}`;
    ...
    await image.writeAsync(path.resolve(process.cwd(), `./output/${imgName}`));
  } catch (error) {
    console.log(error);
  }
};
